...
# glow は単体でも markdown ブラウザとして使う (prefix → Alt+M)

# live-server: HTML の確認用。保存のたびにブラウザを自動リロード
npm i -g live-server
# 使い方: live-server --port=8080 <dir>   # Claude が HTML を書き直すそばから反映
```
...
| prefix → `Alt+M` | glow (markdown ブラウザ) を popup で |
